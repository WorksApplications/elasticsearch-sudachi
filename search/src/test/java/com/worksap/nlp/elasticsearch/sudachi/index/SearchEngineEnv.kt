...
import com.worksap.nlp.search.aliases.Index
...
class SearchEngineEnv(vararg components: String = arrayOf("system")) : ExternalResource() {
  ...
  fun indexAnalyzers(settings: Settings): IndexAnalyzers {
    val indexSettings = IndexSettingsModule.newIndexSettings(index(settings), settings)
  ...
  fun tokenizers(settings: Map<String, String>): Map<String, TokenizerFactory> {
    ...
    return analysisRegistry.buildTokenizerFactories(
        IndexSettingsModule.newIndexSettings(index(indexSettings), indexSettings))
  }

  private fun index(settings: Settings): Index {
    return Index("test", "_na_")
  }
